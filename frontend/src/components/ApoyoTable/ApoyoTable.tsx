...
import type { ApoyoEconomico } from '@models/GestionDirectiva';

const ITEMS_PER_PAGE = 8;

type SortField = keyof ApoyoEconomico;
type SortDirection = 'asc' | 'desc';

interface ApoyoTableProps {
  data: ApoyoEconomico[];
  isLoading: boolean;
  selectedPeriod?: string | null;
}

export const ApoyoTable = ({ data, isLoading, selectedPeriod }: ApoyoTableProps) => {
  ...
  const filteredData = useMemo(() => {
    ...
    if (search) {
      ...
      result = result.filter((d) =>
        ...
        (d.tipo_evento?.toLowerCase().includes(term) ?? false) ||
        (d.lugar?.toLowerCase().includes(term) ?? false)
      );
    }
    return result;
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200">
              ...
              <th className="px-2 py-2 text-left font-semibold text-gray-700">Nivel</th>
              <th className="px-2 py-2 text-left font-semibold text-gray-700">Lugar</th>
              <th className="px-2 py-2 text-left font-semibold text-gray-700">País</th>
              <th className="px-2 py-2 text-left font-semibold text-gray-700">Evento</th>
            ...
            {paginatedData.length === 0 ? (
              ...
              paginatedData.map((row) => (
                <tr key={row.id} className="border-b border-gray-100 hover:bg-[#FFF0F0] transition-colors">
                  ...
                  <td className="px-2 py-2 text-gray-700">{row.nivel || '-'}</td>
                  <td className="px-2 py-2 text-gray-700">{row.lugar || '-'}</td>
                  <td className="px-2 py-2 text-gray-700">{row.pais || '-'}</td>
                  <td className="px-2 py-2 text-gray-700">{row.tipo_evento || '-'}</td>
                  <td className="px-2 py-2 text-gray-700">${Number(row.apoyo_economico).toLocaleString()}</td>
                </tr>
              ))
            )}
          </tbody>
