...
import type { ProgramacionCurso } from '@models/GestionDirectiva';

const ITEMS_PER_PAGE = 8;

type SortField = keyof ProgramacionCurso;
type SortDirection = 'asc' | 'desc';

interface ProgramacionTableProps {
  data: ProgramacionCurso[];
  isLoading: boolean;
  selectedPeriod?: string | null;
}

export const ProgramacionTable = ({ data, isLoading, selectedPeriod }: ProgramacionTableProps) => {
  ...
  const filteredData = useMemo(() => {
    ...
    if (search) {
      ...
      result = result.filter((d) =>
        ...
        (d.profesor?.toLowerCase().includes(term) ?? false)
      );
    }
    return result;
          ...
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200">
              <th className="px-2 py-2 text-left font-semibold text-gray-700">
                <button onClick={() => handleSort('profesor')} className="hover:text-[#CC1C1C]">Profesor{sortIcon('profesor')}</button>
              </th>
              <th className="px-2 py-2 text-left font-semibold text-gray-700">
                <button onClick={() => handleSort('curso')} className="hover:text-[#CC1C1C]">Curso{sortIcon('curso')}</button>
              </th>
              <th className="px-2 py-2 text-left font-semibold text-gray-700">Horario</th>
              <th className="px-2 py-2 text-left font-semibold text-gray-700">Salón</th>
              <th className="px-2 py-2 text-left font-semibold text-gray-700">Inscritos</th>
              <th className="px-2 py-2 text-left font-semibold text-gray-700">Cancelados</th>
            </tr>
          </thead>
          <tbody>
            {paginatedData.length === 0 ? (
              ...
              paginatedData.map((row) => (
                <tr key={row.id} className="border-b border-gray-100 hover:bg-[#FFF0F0] transition-colors">
                  <td className="px-2 py-2 text-gray-700">{row.profesor || '-'}</td>
                  <td className="px-2 py-2 font-medium text-gray-900">{row.curso}</td>
                  <td className="px-2 py-2 text-gray-700">{row.horario || '-'}</td>
                  <td className="px-2 py-2 text-gray-700">{row.salon || '-'}</td>
                  <td className="px-2 py-2 text-gray-700">{row.matriculados}</td>
                  <td className="px-2 py-2 text-gray-700">{row.cancelaron}</td>
                </tr>
              ))
            )}
          </tbody>
