...
import type { PosgradoIndicatorTableProps, SortField, SortDirection } from './PosgradoIndicatorTable.types';
import type { PosgradoIndicator } from '@models/PosgradoIndicator';
...
interface PosgradoTableExtendedProps extends PosgradoIndicatorTableProps {
  onImportComplete?: () => void;
  selectedPeriod?: string | null;
}

export const PosgradoIndicatorTable = ({
  data,
  isLoading,
  tipoPrograma,
  onImportComplete,
  selectedPeriod,
}: PosgradoTableExtendedProps) => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const preview = await uploadPreview(file, 'POSGRADO');
  ...
  const handleConfirmImport = async () => {
    ...
    setIsImporting(true);
    try {
      const result = await confirmImport(importPreview.pending_rows, 'POSGRADO');
      setShowModal(false);
  ...
  const isMaestria = tipoPrograma === 'MAESTRIA';
        ...
        <table className="w-full text-sm">
            ...
            <tr className="bg-gray-50 border-b border-gray-200">
              ...
              <th className="px-3 py-3 text-left font-semibold text-gray-700">
                <button onClick={() => handleSort('matriculados')} className="hover:text-[#CC1C1C]">
                  Matric.{sortIcon('matriculados')}
                </button>
              </th>
              <th className="px-3 py-3 text-left font-semibold text-gray-700">
                <button onClick={() => handleSort('graduados')} className="hover:text-[#CC1C1C]">
                  Grad.{sortIcon('graduados')}
                </button>
              </th>
              <th className="px-3 py-3 text-left font-semibold text-gray-700">
                <button onClick={() => handleSort('desertores')} className="hover:text-[#CC1C1C]">
                  Deser.{sortIcon('desertores')}
                </button>
              </th>
              <th className="px-3 py-3 text-left font-semibold text-gray-700">
                <button onClick={() => handleSort('promedio_acumulado')} className="hover:text-[#CC1C1C]">
                  Promedio{sortIcon('promedio_acumulado')}
                </button>
              </th>
              <th className="px-3 py-3 text-left font-semibold text-gray-700">
                <button onClick={() => handleSort('hombres')} className="hover:text-[#CC1C1C]">
                  Hombres{sortIcon('hombres')}
                </button>
              </th>
              <th className="px-3 py-3 text-left font-semibold text-gray-700">
                <button onClick={() => handleSort('mujeres')} className="hover:text-[#CC1C1C]">
                  Mujeres{sortIcon('mujeres')}
                </button>
              </th>
              <th className="px-3 py-3 text-left font-semibold text-gray-700">
                <button onClick={() => handleSort('empleados')} className="hover:text-[#CC1C1C]">
                  Empleados{sortIcon('empleados')}
                </button>
              </th>
              <th className="px-3 py-3 text-left font-semibold text-gray-700">
                <button onClick={() => handleSort('desempleados')} className="hover:text-[#CC1C1C]">
                  Desemple.{sortIcon('desempleados')}
                </button>
              </th>
              <th className="px-3 py-3 text-left font-semibold text-gray-700">
                <button onClick={() => handleSort('ponencias')} className="hover:text-[#CC1C1C]">
                  Ponencias{sortIcon('ponencias')}
                </button>
              </th>
              <th className="px-3 py-3 text-left font-semibold text-gray-700">
                <button onClick={() => handleSort('publicaciones')} className="hover:text-[#CC1C1C]">
                  Public.{sortIcon('publicaciones')}
                </button>
              </th>
              {isMaestria && (
                <>
                  <th className="px-3 py-3 text-left font-semibold text-gray-700">Proy. Desarr.</th>
                  <th className="px-3 py-3 text-left font-semibold text-gray-700">Proy. Final.</th>
                </>
              )}
            </tr>
          </thead>
          <tbody>
            {paginatedData.length === 0 ? (
              <tr>
                <td colSpan={isMaestria ? 13 : 11} className="px-4 py-12 text-center text-gray-500">
                  No se encontraron registros con los filtros aplicados.
                </td>
              </tr>
            ) : (
              paginatedData.map((row: PosgradoIndicator) => (
                <tr
                  key={row.id}
                ...
                >
                  ...
                  <td className="px-3 py-3 text-gray-700">{row.matriculados}</td>
                  <td className="px-3 py-3 text-gray-700">{row.graduados}</td>
                  <td className="px-3 py-3 text-gray-700">{row.desertores}</td>
                  <td className="px-3 py-3 font-medium text-[#1565C0]">
                    {Number(row.promedio_acumulado).toFixed(2)}
                  </td>
                  <td className="px-3 py-3 text-gray-700">{row.hombres}</td>
                  <td className="px-3 py-3 text-gray-700">{row.mujeres}</td>
                  <td className="px-3 py-3 text-gray-700">{row.empleados}</td>
                  <td className="px-3 py-3 text-gray-700">{row.desempleados}</td>
                  <td className="px-3 py-3 text-gray-700">{row.ponencias}</td>
                  <td className="px-3 py-3 text-gray-700">{row.publicaciones}</td>
                  {isMaestria && (
                    <>
